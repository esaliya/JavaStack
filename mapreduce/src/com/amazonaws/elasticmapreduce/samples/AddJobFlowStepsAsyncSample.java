...
public class AddJobFlowStepsAsyncSample {
	...
	public static void main(String... args) {
		...
		AmazonElasticMapReduceAsync service = new AmazonElasticMapReduceAsyncClient(
				accessKeyId, secretAccessKey, config, executor);
		...
		requestOne.setJobFlowId("sss");
		List<StepConfig> steps = new LinkedList<StepConfig>();
		StepConfig stepConfig = new StepConfig();

		HadoopJarStepConfig value = new HadoopJarStepConfig();
		value.setJar("s3n://b534/Hadoopv11.jar");
		value.setMainClass("edu.indiana.extreme.HadoopRayTracer");
		
		List<String> list = new LinkedList<String>();
		list.add("input dir");
		list.add("output dir");
		value.setArgs(list);
		stepConfig.setHadoopJarStep(value);
		
		requestOne.setSteps(steps);
	 ...
	 */
	public static void invokeAddJobFlowSteps(
			AmazonElasticMapReduceAsync service,
			List<AddJobFlowStepsRequest> requests) {
		...
		for (Future<AddJobFlowStepsResponse> future : responses) {
			...
			try {
				...
				AddJobFlowStepsRequest originalRequest = requests.get(responses
						.indexOf(future));
				...
				if (e.getCause() instanceof AmazonElasticMapReduceException) {
					AmazonElasticMapReduceException exception = AmazonElasticMapReduceException.class
							.cast(e.getCause());
