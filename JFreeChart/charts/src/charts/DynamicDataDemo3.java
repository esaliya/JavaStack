...
public class DynamicDataDemo3 extends ApplicationFrame implements ActionListener {
    ...
    public DynamicDataDemo3(final String title) {
        ...
        for (int i = 0; i < SUBPLOT_COUNT; i++) {
            ...
            TimeSeries series = new TimeSeries(("wow" +  i));
...
//            rangeAxis.setAutoRange(true);
            ...
            subplot.setForegroundAlpha(0.65f);
            plot.add(subplot);
            plot.setGap(20.0);
//            subplot.setBackgroundPaint(Color.blue);
//            subplot.setBackgroundAlpha(0.3f);
        }
  ...
  //      plot.setAxisOffset(new Spacer(Spacer.ABSOLUTE, 4, 4, 4, 4));
    ...
    public void actionPerformed(final ActionEvent e) {
        ...
        if (e.getActionCommand().equals("ADD_ALL")) {
            ...
            System.out.println("Now = " + now.toString());
            for (int i = 0; i < SUBPLOT_COUNT; i++) {
                this.lastValue[i] = 100 * Math.random();
                this.datasets[i].getSeries(0).add(new Millisecond(), this.lastValue[i]);
            }
        }
    ...
    public void autoUpdate(int refreshTime) {
        while (! stop) {
            int i =0;
            for (i = 0; i < SUBPLOT_COUNT; i++) {
                this.lastValue[i] = 100 * Math.random();
                this.datasets[i].getSeries(0).add(new Millisecond(), this.lastValue[i]);
            }
            chart.setTitle("" + Math.random());
            try {
